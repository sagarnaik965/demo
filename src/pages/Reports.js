...
const Reports = () => {
  ...
  let [quarterlyQuarter, setQuarterlyQuarter] = useState("01");
  ...
  const monthlyreportsummary = (event) => {
    ...
    let type = "custom"
    const txnforNo = () => {
      fetch("http://localhost:8082/dept/monthly/" + MonthlyMonth + "/" + MonthlyYear )
        .then((data) => {
          const res = data.json();
          return res
        }).then((res) => {
  ...
  const yealryreportsummary = (event) => {
    ...
    let type = "sixmonth"
    const txnforNo = () => {
      fetch("http://localhost:8082/dept/yearly/" + yearlyYear  )
  ...
  const customreportsummary = (event) => {
    ...
    let type = "other"
    const txnforNo = () => {
      fetch("http://localhost:8082/dept/custom/" + fdate + "/" + ldate)
  ...
  return (
    <div>
      ...
      <div className="bg-light-white-500 px-3 md:px-8 h-40" >
        <Alert color="grey"><Heading2>Reports</Heading2></Alert>
        ...
        <Grid container spacing={1}>
          <Grid item xs={4}>
            <div>
              ...
              >
                <option value="01">January-March</option>
